...
class ElabFTWUIHandler:
    ...
    def add_multiple_links_button_clicked(self, widget: Declarative.UIWidget):
        ...
        # The dialog needs to be called on UIThread
        dialog = Declarative.construct(document_controller.ui, document_controller, self.ui_handler.ui_view, self.ui_handler, self.finishes)
...
class ElabFTWUI:
    ...
    def __create_ui_view(self, ui: Declarative.DeclarativeUI) -> dict:
        ...
        add_tag_text_edit = ui.create_text_edit(
            ...
            height=22,
            on_return_pressed='add_tag_return_pressed')
        add_tag_button = ui.create_push_button(name='add_tag_button', text='Add Tag', on_clicked='add_tag_button_clicked')
        add_tag_row = ui.create_row(add_tag_text_edit, add_tag_button)

        add_link_text_edit = ui.create_text_edit(
            name='add_link_text_edit',
            text='@binding(add_link_text)',
            placeholder_text=' ELN item id ',\
            height=22,
            on_return_pressed='add_link_return_pressed')
        add_link_button = ui.create_push_button(name='add_link_button', text='Add to linked items', on_clicked='add_link_button_clicked')
        add_link_row = ui.create_row(add_link_text_edit, add_link_button)
        add_multiple_links_button = ui.create_push_button(
            name='add_multiple_links_button',
            text='Dialog: Add multiple links (not fully supported by eLabFTW)',
            on_clicked='add_multiple_links_button_clicked')
